jQuery(function($) {
            ...
            $.each(data.rows, function(index, item) {
                ...
                content += "<tr class='simple-row'>" +
                    ...
                    "<td colspan='8'> <div class='table-detail'> <div class='row'> <div class='col-xs-12 col-sm-2'> <div class='text-center'>" +
                        "" + $ctx + "<img height='150' class='thumbnail inline no-margin-bottom' alt='头像' style='width: 136px; height: 150px;' src='/statics/"+ (item.headImg ? "file/head-img-file/" + item.headImg : "assets/avatars/profile-pic.jpg") +"' />" +
    ...
    /*绑定新增用户信息表单验证*/
    $("body").delegate("#username-i", "blur", function() {
        var $this = $(this);
        if ($this.val()) {
            /*验证用户名是否唯一*/
    ...
    $("#insert").click(function() {
        layer.confirm(
            "<form class='form-horizontal' id='user-form' style='height: 240px;'>" +
            "<div class='row'>" +
            "<div class='col-sm-12'>" +
            ...
            "<input type='text' id='username-i' class='width-100' />" +
            ...
            "<div class='space-12' style='margin: 20px 0;'></div>" +
            ...
            "<div class='col-sm-7'>" +
            "<input type='text' id='level-i' class='width-100' />" +
            ...
            "<div class='col-sm-7'>" +
            "<label>" +
            "<input type='checkbox' id='state-i' class='ace ace-switch ace-switch-6'/>" +
            ...
            "</form>",
            {
                ...
                if (!$("#username-i").closest(".form-group").hasClass("has-success")) {
                    $("#username-i").focus();
                    return;
                }
                var param = {username: $("#username-i").val(),
                    level: $("#level-i").val(),
                    state: $("#state-i").get(0).checked ? 1:0}
                $.post($ctx + "/user/insertUser", param, function(data) {
                    if (data == "success") {
                        $.modalMsg("添加成功","success");
                        /*重置表单*/
    ...
    $("#simple-table").delegate(".updateUser", "click", function() {
        var id = this.getAttribute("data-id");
        var success = false, data_ = {};
        $.ajax({url: $ctx + "/user/findUserById", type: 'GET', async: false, data: {id: id}, success: function(data) {
            success = true, data_ = data;
        }, dataType: "JSON"});
        if (!success) return;
        layer.confirm(
            "<form class='form-horizontal' id='user-form-update' style='height: 120px;'>" +
            "<input id='userId-u' value='"+ data_.id +"' style='display: none;' />" +
            "<div class='col-sm-12'>" +
            "<div class='form-group has-success'>" +
            "<label for='username-update' class='col-sm-3 control-label no-padding-right'>用户名称：</label>" +

            "<div class='col-sm-7'>" +
            "<label id='username-u' class='control-label width-100' style='text-align: left;'>"+ data_.username +"</label>" +
            "</div>" +
            "<div class='help-block col-sm-reset inline'></div>" +
            "</div>" +
            "</div>" +
            ...
            "<div class='col-sm-7'>" +
            "<span class='block'>" +
            "<input type='text' id='level-u' placeholder='"+ data_.level +"' class='width-100' />" +
            "</span>" +
            "</div>" +
            "<div class='help-block col-sm-reset inline'></div>" +
            "</div>" +
            ...
            "<div class='col-sm-7'>" +
            "<label>" +
            "<input type='checkbox' id='state-u' class='ace ace-switch ace-switch-6'"+ (data_.state == 1 ? " checked":"") +"/>" +
            "<span class='lbl' style='margin-top: 5px;'><label for='state-update'>&nbsp;&nbsp;&nbsp;"+ (data_.state == 1 ? "启用":"禁用") +"</label></span>" +
            "</label>" +
            "</div>" +
            "<div class='help-block col-sm-reset inline'></div>" +
            "</div>" +
