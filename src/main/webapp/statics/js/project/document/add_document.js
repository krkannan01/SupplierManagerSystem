...
jQuery(function($) {
    ...
    initSupplierSelectBox($("select[name=supplier]"));
...
// 初始化供应商选择框
function initSupplierSelectBox(el) {
    var $el = $(el);
    var data = {};
    var success = false;
    // 加载数据
    $.ajax({
        url: get_supplier_part_list_path,
        type: "GET",
        dataType: "JSON",
        async: false,
        success: function(result) {
    ...
    if (data != null && data.length > 0) {
        ...
        html = "<option value='0'> 无信息 </option>";
    }
    $el.html(html);
}
...
function add() {
    ...
    formdata.append("supplierId", $("select[name=supplier]").val());
