...
(function (factory) {
    ...
    var Tree = function Tree(element, options) {
        ...
        if (this.options.multiSelect) {
            if (this.options.itemSelect) {this.$element.on('click.fu.tree', '.tree-item > span', $.proxy(function (ev) {
                ...
                if (!$currentTarget.is('li')) {
                    if ($currentTarget.parent().next().children('li').length === 0) {
                        if (!this.options.folderSelect) this.discloseFolder($currentTarget.parent(), true);
                    } else {
                        this.selectItems($currentTarget);
                    }
                } else {
                    //如果是子节点，判断当前是否全部选中来决定父级是否选中
                    if ($currentTarget.parent().find('li').length == $currentTarget.parent().find('li.tree-selected').length) {
            ...
            //ACE
            this.$element.on('click.fu.tree', '.tree-branch-header > span > span', $.proxy(function (ev) {
                // TODO
            ...
            if (this.options.itemSelect) {this.$element.on('click.fu.tree', 'li.tree-item', $.proxy(function (ev) {
    ...
    Tree.prototype = {
        ...
        selectItems: function selectItems(el) {
            if (this.options.itemSelect) {
                ...
                var $this = this;
                el.parent().next().children("li").each(function(index, item) {
