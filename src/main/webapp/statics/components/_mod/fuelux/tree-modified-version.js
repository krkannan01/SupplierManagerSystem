...
(function (factory) {
	...
	var Tree = function Tree(element, options) {
		...
		if (this.options.itemSelect) {
			if (!this.options.expand) {
				...
				this.$element.on('click.fu.tree', '.tree-item > span', $.proxy(function (ev) {
					var $currentTarget = $(ev.currentTarget).parent();
					this.selectItem($currentTarget);
					//this.selectItems();
					//如果是文件夹节点
					if (!$currentTarget.is('li')) {
						//判断是否是首次打开，because:首次需要加载，需要在加载时动手脚，if 已经加载过的，the 只需要添加样式就行
						if ($currentTarget.parent().next().children('li').length === 0) {
							if (!this.options.folderSelect) {
								this.discloseFolder($currentTarget.parent(), true);
							}
						} else {
							this.selectItems($currentTarget);
						}
					} else {
						//如果是子节点，判断当前是否全部选中来决定父级是否选中
						if ($currentTarget.parent().find('> li.tree-item').length == $currentTarget.parent().find('> li.tree-selected').length) {
							this.selectItem($currentTarget.parent().prev().children('span').eq(0), 'select');
						} else {
							this.selectItem($currentTarget.parent().prev().children('span').eq(0), 'deselect');
						}
					}
				}, this));
			}
		...
		if (!this.options.expand) {
			...
			//ACE
			this.$element.on('click.fu.tree', '.tree-branch-header > span > i', $.proxy(function (ev) {
				// 展开当前节点
				if (!this.options.folderSelect) this.toggleFolder($(ev.currentTarget).parent().parent());
			}, this));
		}
	...
	Tree.prototype = {
		...
		populate: function populate($el, isBackgroundProcess, allSelect_flag) {
			var self = this;
			...
			this.options.dataSource(treeData ? treeData : {}, function (items) {
				$.each(items.data, function (index, value) {
					...
					if (value.type === 'folder') {
						...
						var header = expand ? $entity.find('.tree-branch-header > span > i') : $entity.find('.tree-branch-header');
						...
						if (value.value) {
							$entity.find("> .tree-branch-header > .tree-item").attr("data-value", value.value);
						}
						// 改动结束
		...
		selectTreeNode: function selectTreeNode(clickedElement, nodeType, notSync_flag) {
			...
			if ( this.options.multiSelect ) {
				if (notSync_flag === 'select') {
					multiSelectNodes(this, clicked, selected);
				} else if (notSync_flag === 'deselect'){
					multiDeselectNodes(this, clicked, selected);
				} else {
		...
		discloseFolder: function discloseFolder(el, allSelect_flag) {
			var $el = $(el);
			...
			if (!$treeFolderContent.children().length) {
				this.populate($treeFolderContent, false, allSelect_flag);
			}
		...
		selectItem: function selectItem(el, notSync_flag) {
			if (this.options.itemSelect) {
				this.selectTreeNode(el, 'item', notSync_flag);
			}
		},

		selectItems: function selectItems(el) {
			if (this.options.itemSelect) {
				var flag = el.hasClass('tree-selected') ? "select":"deselect";
				var $this = this;
				el.closest("li[role=treeitem]").find(".tree-item").each(function(index, item) {
					$this.selectTreeNode($(item), 'item', flag);
				});
			}
		},
	...
	function styleNodeSelected (self, $element, $icon) {//ACE
		...
		*/
		...
		if ( $element.hasClass("tree-item") && $icon.hasClass(self.options['unselected-icon']) ) {//ACE
	...
	function styleNodeDeselected (self, $element, $icon) {//ACE
		...
		*/
		...
		if ( $element.hasClass("tree-item") && $icon.hasClass(self.options['selected-icon']) ) {//ACE
			$icon.removeClass(self.options['selected-icon']).addClass(self.options['unselected-icon']); // make bullet
		}
	}

	function multiSelectNodes (self, clicked, selected) {
	...
	function multiDeselectNodes (self, clicked, selected) {
		// search for currently selected and add to selected data list if needed
		$.each(selected.$elements, function (index, element) {
			var $element = $(element);
			if ($element[0] !== clicked.$element[0]) {
